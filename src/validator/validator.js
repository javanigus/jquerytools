		...
		'default' : [
			...
			function(errs) {
				...
				$.each(errs, function(i, err) {
					// add error class
					var input, msg, name;
					$.each(err.input, function(j, ins) {
						input = $(ins);
						input.addClass(conf.errorClass);
						// get handle to the error container
						if (input.data("msg.el") === null) {
			...
			invalidate: function(errs, e) {
				...
				if (!e) {
					...
					$.each(errs, function(key, val) {
						// handle regular inputs and multi-value inputs (checkboxes, multiselects)
						var input = inputs.filter("[name='" + key + "'], [name='" + key + "[]']");
						if (input.length) {
